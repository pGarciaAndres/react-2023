...
import { Skill } from '../store/skills/reducer'
...
export const SkillRow: React.FC<Props> = ({ skill }) => {
    ...
    const [skillYears, setSkillYears] = useState<string>(skill.years)
    const { updateSkill, removeSkill } = useSkills()

    const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        event.currentTarget.name === 'name'
            ? setSkillName(event.target.value)
            : setSkillYears(event.target.value)
    }

    const handleUpdateSkillName = () => {
        updateSkill({ id: skill.id, name: skillName, years: skillYears })
        setEdit(false)
    }
        ...
        <TableRow>
            ...
            >
                {!edit ? (
                    ...
                    <button type="button" onClick={handleUpdateSkillName}>
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
